/**
 * L9.4 — Sequence Cycle Detector
 *
 * §9.4.4 — Runtime acyclicity law. Any cyclic dependency must be
 * rejected before runtime (INV-9.4-A). Implemented as iterative
 * three-colour DFS so there is no recursion-depth limit and the
 * traversal is deterministic (§9.4.16).
 */

import type { L9DagEdge } from './sequence-dag-edge';

export interface L9CycleDetectionResult {
  readonly acyclic: boolean;
  readonly cycles: readonly (readonly string[])[];
}

export function detectL9Cycles(
  nodeIds: readonly string[],
  edges: readonly L9DagEdge[],
): L9CycleDetectionResult {
