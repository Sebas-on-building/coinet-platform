/**
 * L9.4 — Deterministic Topological Sort
 *
 * §9.4.16.3 — Kahn's algorithm with stable (stage, node_id) tie-break.
 * Given identical DAGs the output order is identical, which is what
 * gives the L9 runtime replay stability.
 */

import { L9DagNode, L9_STAGE_INDEX } from './sequence-dag-node';
import type { L9DagEdge } from './sequence-dag-edge';

export interface L9ToposortResult {
  readonly order: readonly string[];
  readonly ok: boolean;
  readonly reason: string | null;
}

export function l9Toposort(
  nodes: readonly L9DagNode[],
  edges: readonly L9DagEdge[],
): L9ToposortResult {
  const byId = new Map<string, L9DagNode>();
...
function sortReady(ready: L9DagNode[]): void {
  ready.sort((a, b) => {
