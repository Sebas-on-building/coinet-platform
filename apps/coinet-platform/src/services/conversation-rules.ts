...
- Market-wide ("how's the market?") → the engine has no market-wide judgment path yet. Speak to the per-token regime context the engine provides (Fear & Greed, regime) but say plainly that a full market verdict isn't something you produce yet. No improvised market thesis.
...
- Only discuss coins/tokens the user explicitly asked about. Never inject coins from training data, prior turns, or context that don't match the query.
`;
