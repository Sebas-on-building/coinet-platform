...
/**
 * Basic Signal and Alert System Demo
 *
 * This script demonstrates the core signal processing and alert system
 * functionality in a simplified manner.
 */
...
async function demoSignalAlertSystem() {
  ...
  try {
    ...
    const binanceSource = await prisma.signalSource.upsert({
      where: { name: 'Binance Exchange' },
      update: {},
      create: {
    ...
    console.log(`✅ Signal source: ${binanceSource.name}\n`);

    // 2. Create signals
    console.log('📈 Creating and processing signals...');

    const btcSignal = await prisma.signal.create({
      data: {
    ...
    console.log(`✅ Created signal: ${btcSignal.signalType} for ${btcSignal.symbol}\n`);

    // 3. Create alert triggers (without complex relationships)
    console.log('⚠️ Creating alert triggers...');

    const alertTrigger = await prisma.alertTrigger.create({
      data: {
        alertId: 'demo-alert-123', // Using a demo ID since alerts don't exist
        tenantId: 'default',
    ...
    console.log(`✅ Created alert trigger: ${alertTrigger.result}\n`);

    // 4. Create alert performance tracking
    console.log('📊 Creating alert performance tracking...');

    const alertPerformance = await prisma.alertPerformance.create({
      data: {
        alertId: 'demo-alert-123',
        tenantId: 'default',
        userId: 'demo-user-123',
        alertTriggerId: alertTrigger.id,
        outcome: 'NEUTRAL', // Alert didn't trigger
        kpiValue: 45000.50,
    ...
    // 6. Query analytics and insights
    ...
    signalSources.forEach(source => {
      console.log(`   ${source.name}: ${source.sourceType} - ${source.isActive ? 'Active' : 'Inactive'}`);
    });
