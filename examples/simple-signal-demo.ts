...
/**
 * Simple Signal and Alert System Demo
 *
...
async function demoSignalAlertSystem() {
  ...
  try {
    ...
    // 1. Get or create signal sources
    console.log('📡 Getting signal sources...');

    let binanceSource = await prisma.signalSource.findFirst({
      where: { name: 'Binance Exchange' }
    });

    if (!binanceSource) {
      binanceSource = await prisma.signalSource.create({
        data: {
    ...
    console.log(`✅ Created signal source: ${binanceSource.name}\n`);
