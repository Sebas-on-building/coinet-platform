...
// Interface for Crypto.news RSS item
...
/**
 * Adapter for Crypto.news RSS feed
 */
export class CryptoNewsAdapter extends BaseNewsSourceAdapter {
  private rssUrl: string = "https://crypto.news/feed/";
  private categoryEndpoints: Record<string, string> = {
    bitcoin: "https://crypto.news/bitcoin/feed/",
    ethereum: "https://crypto.news/ethereum/feed/",
    blockchain: "https://crypto.news/blockchain/feed/",
    regulation: "https://crypto.news/regulation/feed/",
    defi: "https://crypto.news/defi/feed/",
    nft: "https://crypto.news/nft/feed/",
    markets: "https://crypto.news/market-insights/feed/",
  };
  ...
  /**
   * Convert Crypto.news RSS item to standardized NewsItem format
   */
  private convertToNewsItem(item: CryptoNewsItem): NewsItem {
    ...
    const newsItem: NewsItem = {
      ...
      url: item.link,
      image_url: imageUrl,
      author: item.creator || "Crypto.news",
        ...
        market_sentiment:
          impactScore > 0.2
            ...
            : impactScore < -0.2
              ? "bearish"
              : "neutral",
        importance: 0.75, // Crypto.news is a significant source
      ...
      related_news: [],
      fact_checking: {
        verified_by: ["Crypto.news Editorial"],
        accuracy_score: 0.82,
        disputed_claims: [],
      },
      verified: true,
      verification_sources: ["Crypto.news"],
    };
   ...
   */
  private async parseRSS(xml: string): Promise<CryptoNewsItem[]> {
   ...
   */
  private async fetchRSS(url: string): Promise<CryptoNewsItem[]> {
    try {
  ...
  private getUrlsToFetch(filter?: Partial<NewsFilter>): string[] {
    ...
    if (filter.categories && filter.categories.length > 0) {
      for (const category of filter.categories) {
        ...
        } else if (category === "market" && this.categoryEndpoints["markets"]) {
          urls.push(this.categoryEndpoints["markets"]);
        } else if (
          category === "technology" &&
  ...
  async fetchNews(filter?: Partial<NewsFilter>): Promise<NewsItem[]> {
    try {
      ...
      console.error("Error fetching news from Crypto.news:", error);
      return [];
    }
  }
